...
export default async function SettingsPage() {
  ...
  const cards = [
    { label: "Tác phẩm đang đọc", value: activeStories.count ?? 0, icon: BookOpen },
    { label: "Đã lưu trữ", value: archivedStories.count ?? 0, icon: Archive },
    { label: "Bản nháp chờ review", value: pendingImports.count ?? 0, icon: FileClock },
  ];
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 sm:py-8">
      <div>
        ...
        <h1 className="mt-2 text-4xl font-extrabold tracking-tight sm:text-5xl">Cài đặt</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6" style={{ color: "var(--kd-text-muted)" }}>
          Nơi gom các tuỳ chọn cấp ứng dụng. Tuỳ chỉnh chữ khi đọc vẫn nằm trong nút Aa của Reader.
        </p>
      </div>
      ...
      >
        ...
        <p className="mt-1 text-sm" style={{ color: "var(--kd-text-muted)" }}>
          Đổi giao diện Library/Search/Settings. Reader vẫn có theme đọc riêng để không làm mất nhịp đọc.
        </p>
