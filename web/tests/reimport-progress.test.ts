...
import { parseChapterIdPairs, resolveRemapTarget, type ChapterIdPair } from "../lib/import/reimport-progress";
...
describe("parseChapterIdPairs", () => {
  ...
  it("filters out malformed entries", () => {
    ...
    expect(parseChapterIdPairs(raw)).toEqual([{ oldChapterId: "a", newChapterId: "b" }]);
  });
});
...
describe("resolveRemapTarget", () => {
  it("resolves a directly matched (unmerged) chapter to itself", () => {
    const pairs: ChapterIdPair[] = [
      { oldChapterId: "c1", newChapterId: "draft-1", oldRevisionId: "r1", newRevisionId: "r2", contentChanged: true },
    ];

    const target = resolveRemapTarget(pairs, "c1");

    expect(target).toEqual({ oldRevisionId: "r1", targetChapterId: "c1", targetRevisionId: "r2" });
  });

  it("chases a merged chapter through to its primary's real chapter/revision id", () => {
    const pairs: ChapterIdPair[] = [
      { oldChapterId: "primary-old", newChapterId: "draft-1", oldRevisionId: "r-primary", newRevisionId: "r-new", contentChanged: true },
      { oldChapterId: "merged-old", newChapterId: "draft-1", oldRevisionId: "r-merged", newRevisionId: null, contentChanged: true, merged: true },
    ];

    // The user's progress was on the chapter that got merged away.
    const target = resolveRemapTarget(pairs, "merged-old");

    expect(target).toEqual({ oldRevisionId: "r-merged", targetChapterId: "primary-old", targetRevisionId: "r-new" });
  });
