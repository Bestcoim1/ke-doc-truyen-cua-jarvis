...
describe("resolveResumeAnchor", () => {
  it("resolves exact anchor id match", () => {
    const newBlocks = [block("Đoạn một."), block("Đoạn hai."), block("Đoạn ba.")];
    const oldAnchorId = `p_${fingerprintParagraph("Đoạn hai.")}`;

    const result = resolveResumeAnchor(oldAnchorId, fingerprintParagraph("Đoạn hai."), 1, 3, newBlocks);
  ...
  it("falls back to fingerprint match when the anchor id itself is gone but the paragraph survives", () => {
    ...
    const result = resolveResumeAnchor(staleAnchorId, oldFingerprint, 0, 1, newBlocks);
  ...
  it("falls back to ordinal ratio when no fingerprint survives", () => {
    const newBlocks = Array.from({ length: 10 }, (_, i) => block(`Đoạn mới ${i}.`));

    // old chapter had 20 paragraphs, reader was at ordinal 10 (halfway).
    const result = resolveResumeAnchor("p_stale", "deadbeefdead", 10, 20, newBlocks);
  ...
  it("clamps the ordinal fallback into range and handles oldBlockCount=0", () => {
    ...
    const result = resolveResumeAnchor("p_stale", "deadbeefdead", 0, 0, newBlocks);
